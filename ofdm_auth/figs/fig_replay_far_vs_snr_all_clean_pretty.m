function fig_replay_far_vs_snr_all_clean_pretty()
...
if isinf(best_FRR_D)
    warning('No tauD met FAR target; using approximate EER-like fallback.');
    best_gap = inf;
    for tau = tauD_grid
        cfgTmp = cfgD;
        cfgTmp.tauD = tau;
...
for si = 1:numel(SNR_list)
    ...
    fprintf('SNR=%g dB: Replay FAR(P)=%.4f FAR(IM)=%.4f FAR(D)=%.4f\n', ...
        snr, replay_FAR_P(si), replay_FAR_IM(si), replay_FAR_D(si));
end
...
plot(SNR_list, max(replay_FAR_P,  1/MC), '-o', ...
    'LineWidth', 2.2, 'MarkerSize', 8);
plot(SNR_list, max(replay_FAR_IM, 1/MC), '-s', ...
    'LineWidth', 2.2, 'MarkerSize', 8);
plot(SNR_list, max(replay_FAR_D,  1/MC), '-d', ...
    'LineWidth', 2.2, 'MarkerSize', 8);

xlabel('SNR_{ref} (dB)', 'FontSize', 14);
ylabel('Probability (log scale)', 'FontSize', 14);
title(sprintf('Replay Robustness: Perm vs IM vs DPPA (L = %d, \\alpha = %.2f)', ...
    cfg.Lsym, cfg.alpha), ...
    'FontSize', 15, 'FontWeight', 'bold');

legend('Replay FAR (Perm)', 'Replay FAR (IM)', 'Replay FAR (DPPA)', ...
