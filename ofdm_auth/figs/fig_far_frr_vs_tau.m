...
for s = 1:length(SNR_list)
    ...
    FAR_s = movmean(FAR, 5);
    FRR_s = movmean(FRR, 5);
    plot(tauP_grid, max(FAR_s(:,1), p_floor), '-', 'LineWidth', 3);
    plot(tauP_grid, max(FRR_s(:,1), p_floor), '--', 'LineWidth', 3);
    ylim([p_floor 1]);
end
...
for s = 1:length(SNR_list)
    ...
    FAR_s = movmean(FAR, 5);
    FRR_s = movmean(FRR, 5);
    plot(tauIM_grid, max(FAR_s(:,1), p_floor), '-', 'LineWidth', 3);
    plot(tauIM_grid, max(FRR_s(:,1), p_floor), '--', 'LineWidth', 3);
    ylim([p_floor 1]);
end
