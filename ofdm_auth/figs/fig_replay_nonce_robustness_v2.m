...
u0 = 3;
SNR_list = [-5 5 15];     % two points is usually enough
MC = 1000;
...
for si = 1:numel(SNR_list)
    ...
    fprintf('SNR=%g dB: Legit FRR(P)=%.4f FRR(IM)=%.4f | Replay FAR(P)=%.4f FAR(IM)=%.4f\n', ...
        snr, legit_FRR_P(si), legit_FRR_IM(si), replay_FAR_P(si), replay_FAR_IM(si));
end
