function fig_frr_vs_puf_flip_all_stacked()

%% =========================
% Common settings
% ==========================
alpha = 0.75;
L_list = [4 64];          % short vs long aggregation
SNR_list = [15];        % two representative SNRs
MC = 6000;
MC_cal = 3000;

%p_list = [0 0.002 0.005 0.01 0.02 0.05];
p_list = [0 0.002 0.005 0.01 0.03];
...
% Easy switch for DPPA alphabet:
cfgD.phi_choices_deg = [-28 -10 10 28];
% Alternative:
% cfgD.phi_choices_deg = [-20 20];
...
tauD_grid = linspace(0.70, 1.00, 151);
...
fprintf('  tauD  = %.4f   (FAR=%.4f, FRR=%.4f), phi_choices = [%s]\n\n', ...
    cfgD.tauD, best_FAR_D, best_FRR_D, num2str(cfgD.phi_choices_deg));
...
fig = figure('Color','w', 'Position', [100 60 880 980]);
...
for k = 1:3
    ...
    xticks(ax, p_list);
    xticklabels({'0','0.002','0.005','0.01','0.02','0.05'});
    ylim(ax, [1/MC 1]);
end
