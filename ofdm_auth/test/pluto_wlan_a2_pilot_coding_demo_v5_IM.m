...
for k = 1:Ntrials
    ...
    try
        ...
        results(k).ok         = true;
        results(k).Tim_H1_auth = res.Tim_H1_auth;
        results(k).Tim_H0_auth = res.Tim_H0_auth;
        results(k).errmsg     = '';
...
%     'VariableNames', {'trial','fc_MHz','ok','BER','TimH1_mean','TimH0_mean','acceptH1','acceptH0','Lsym'});
    T = table( ...
        ...
        [results(ok_idx).ok].', ...
        [results(ok_idx).ber].', ...
        [results(ok_idx).TimH1_mean].', ...
        [results(ok_idx).TimH0_mean].', ...
        [results(ok_idx).Lsym].', ...
        ...
        'VariableNames', {'trial','fc_MHz','ok','BER','TimH1_mean','TimH0_mean', ...
        'Lsym','Lauth','VH1','VH0','acceptH1','acceptH0'} );
    disp(T);
...
tau_grid   = -0.5:0.01:0.9;
alpha_grid = 0.4:0.05:0.95;

S_im = ota_postprocess_method(results, ...
    'IM', ...
    'Tim_H1_auth', ...
    'Tim_H0_auth', ...
    tau_grid, ...
...
save('results_im.mat','results','S_im');
